...
An Agent corresponds directly to a user that exists in the SugarCRM instance you are testing. All users except for the OOTB admin user *must* be created by `Fixtures.create` before you attempt to create a user agent for it. Please refer to [Thorn Fixtures API](#thorn::fixtures) to create the needed users.
